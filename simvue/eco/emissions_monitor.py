...
class CO2Monitor(pydantic.BaseModel):
    ...
    def check_api_arguments(cls, values: dict[str, Any]) -> dict[str, Any]:
        ...
        if not (
            ...
            or values.get("co2_signal_api_token")
            or values.get("offline")
        ):
            raise ValueError(
                "ElectricityMaps API token or hardcoded CO2 "
                "intensity value is required "
                "for emissions tracking.",
            )
        if not isinstance(values.get("thermal_design_power_per_cpu"), float):
            values["thermal_design_power_per_cpu"] = 80.0
            logger.warning(
                "⚠️  No TDP value provided for current CPU, will use "
                "arbitrary value of 80W.",
            )

        if not isinstance(values.get("n_cores_per_cpu"), float):
            values["n_cores_per_cpu"] = 4
            logger.warning(
                "⚠️  No core count provided for current CPU, will use "
                "arbitrary value of 4.",
            )

        if not isinstance(values.get("thermal_design_power_per_gpu"), float):
            values["thermal_design_power_per_gpu"] = 130.0
            logger.warning(
    ...
    def post_init_setup(self) -> typing.Self:
        ...
        if self.co2_intensity:
            logger.warning(
                "⚠️ Disabling online data retrieval, using %s "
    ...
    def check_refresh(self) -> bool:
        ...
        if (
            ...
            and self._data_file_path.exists()
            and (
    ...
    def estimate_co2_emissions(
        ...
        if self.co2_intensity:
            ...
            if self._client:
                ...
                logger.debug(
                    "🗂️ Using data for region '%s' from local "
        ...
        logger.debug(
            "📝 For process '%s', in interval %s, "
            "recorded: CPU=%s, "
            "Power=%skW, Energy = "
            "%s, CO2=%s",
            process_id,
            measure_interval,
            f"{_process.cpu_percentage:.2f}%",
            f"{_process.power_usage:.2f}kW",
            f"{_process.energy_delta}kWh",
            f"{_process.co2_delta:.2e}kg",
        )
        return True
